...
<div className={`flex flex-col lg:flex-row lg:space-x-4 p-2 border-2 rounded-lg m-1 ${editedRecord.summa < 0 ? 'border-rose-600' : 'border-teal-600'}`}>
    <div className="flex flex-col sm:flex-row sm:space-x-4 w-full">
        <div className='flex flex-row'>
            <input
                ...
                placeholder="date"
                onChange={Change}
                className="border border-emerald-400 rounded-md p-2 flex-1 min-w-[150px] mb-2 sm:mb-0 mr-1 h-full"
            ...
            <input
                ...
                placeholder="summa"
                onChange={Change}
                className="border border-orange-600 rounded-md p-2 flex-1 min-w-[100px] min-h-[44px] mb-2 sm:mb-0 mr-1 h-full text-end"
            />
            {changeable ? 
            <button
            onClick={handleEditRecord}
            className="bg-orange-400 border-orange-600 border-2 rounded-md p-2 min-w-[100px] self-start sm:self-auto"
            >
                Edit
            </button>: null}
        </div>
    ...
    <div className="flex flex-col sm:flex-row sm:space-x-4 w-full lg:w-auto">
        {editedRecord.description ? <textarea
            name="description"
            disabled={!changeable}
